...
loginApp.controller("loginController", function($scope, $http,$cookies) {
	...
	$scope.login = function(){
		...
		$http ({
			...
			url : 'https://localhost:8443/StudentCourseRegistrationSystem/rest/students/login',
			data : { 
				"emailId" :   $scope.email,
		...
		.success(function(data,status,headers,config){
			...
			$http.get("https://localhost:8443/StudentCourseRegistrationSystem/rest/students/session").success(function(data, status, headers,config) {
				...
				if($scope.studentId==data.studentId){
					...
					window.location.href = "https://localhost:8443/StudentCourseRegistrationSystem/HTML/Dashboard.html";
						...
						window.location.href = "https://localhost:8443/StudentCourseRegistrationSystem/HTML/AdminDashboard.html";
					}
				}else{
