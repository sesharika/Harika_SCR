		...
		$scope.loggedIn = false;*/
					...
					$scope.getRangeCourses = function() {
						...
						if ($scope.singleSelect == "date") {
							$http({
									method : 'GET',
									url : 'https://localhost:8443/StudentCourseRegistrationSystem/rest/courses?startDate='+$scope.start+'&endDate='+ $scope.end
									})
									.success(function(data, status, headers,config) {
												$scope.courseList = data;
												$scope.status = status;
							...
							$http({
									method : 'GET',
									url : 'https://localhost:8443/StudentCourseRegistrationSystem/rest/courses?startAmount='+$scope.start+'&endAmount='+$scope.end
								})
									.success(function(data, status, headers,config) {
												$scope.courseList = data;
												$scope.status = status;
